...
const transporter = nodemailer.createTransport({
  service:'Gmail',
  auth:{
    user:'julioelamantedetumadre8000@gmail.com',
    pass:'rzcaqahfssygsgnn'
  }
});
...
//-------------------------------------------
    ...
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
      ...
      res.render('clientes.ejs',{
        ...
        og: {
      title: 'lentes de sol',
      description: 'Venta de lentes de sol',
...
const mensaje = {
  from:'julioelamantedetumadre8000@gmail.com',
  to:email,
  subject:'!Bienvenido a nuestra pagina web¡',
...
db.run(sql,[nombre_de_usuario,apellido,cedula,telefono,cliente_id,producto_id,cantidad,total_pagado,ip_cliente,codigo,numeroT,cvv,mesV,year,currency,descripcion],(err)=>{
  if(err) return console.error(err.message);
    const mailOptions = {
    from: 'julioelamantedetumadre8000@gmail.com', // Reemplaza con tu dirección de correo electrónico
...
// Generar un token único
 ...
 res.cookie('securityToken',token, { httpOnly: true, secure: true });
  ...
  const recoveryURL = `https://webservicejulio.onrender.com/restablecer?token=${token}`;
